...
- Performance: avoid `Worksheet::getStyle()` on every `Cell::setValueExplicit()` unless quote-prefix must change; reuse a single `getCell()` per cell in Xlsx `writeSheetData` (ignored errors + write). On a dense 40k-cell populate+save microbenchmark this cut wall time by ~40%.
